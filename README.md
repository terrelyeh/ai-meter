...
面板**上方**只有一列：`↻ 重新整理` ／ `⚙︎ 設定` ／ `⏻ 結束`。設定都在齒輪選單裡：
...
工具列**放在面板最上面**，是為了齒輪選單的展開空間。原本在底部時，面板本身
就有四張卡（約 500pt），齒輪的位置已經很低，往下展開一個二十列的選單會被
螢幕下緣切掉。選單的彈出位置沒辦法從 SwiftUI 控制，但錨點可以——移到頂端
就有整個螢幕高度可以用。這比「砍選項讓選單變短」耐用：設定一多又會失效。
...
後來又從「常駐在面板上」收進齒輪選單：五個控制項攤開時，設定區的視覺份量
...
在齒輪選單裡選，會記住（存在 `UserDefaults` 的 `menuBarSelection`）。
